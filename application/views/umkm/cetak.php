...
<html>
	...
	<title>
   		Cetak Data Usaha Mikro | <?= $j['sebutan_desa'] ?> <?= $j['nama_desa'] ?>
	</title>
	...
	<body>
		...
		<div id="body">
			<div align="center">
				...
				<h3>DATA UMKM <?= strtoupper($j['sebutan_desa'] . ' ' . $n['nama_desa']); ?></h3>
			</div>
			<br>
			<table class="border thick ">
				...
				<tr class="border thick">
					...
					<th class="text-center" width='100'>Nomor Telepon</th>
					<th class="text-center" width='100'>Nama Usaha</th>
					<th class="text-center" width='100'>Kategori</th>
					<th class="text-center" width='120'>Harga/Satuan</th>
					<th class="text-center" width='120'>Alamat</th>
					<th class="text-center" width='120'>Tgl Buat</th>
				</tr>
				</thead>
				<tbody>
					<?php
						$no=1;
						foreach($list_umkm as $data) : 
					?>
					<tr class="data">
						<td class="text-center" width="2"><?= $no++; ?></td>
                        <td><?= strtoupper($data->nama_pemilik); ?></td>
						<td><?= format_telpon($data->no_tlp); ?></td>
						<td><?= strtoupper($data->nama_usaha); ?></td>
						<td><?= strtoupper($data->nama_kat); ?></td>
						<td>Rp. <?= $data->harga_produk; ?>/<?= $data->satuan_produk; ?></td>
						<td><?= strtoupper($data->alamat); ?></td>
						<td><?= tgl_indo($data->tgl_posting); ?></td>
					</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
