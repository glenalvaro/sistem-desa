...
<div class="h-full w-full relative">
    <div class="w-full h-auto flex flex-col gap-7 2xl:px-72 xl:px-64 lg:px-52 md:px-36 sm:px-24 phone:px-10 my-10">
        <div class="w-auto h-auto flex flex-col gap-2 items-center">
            <h1 class="text-center font-semibold text-xl">Data Wilayah <?= $sebutan_desa; ?> <?= $nama_desa; ?></h1>
          ...
          <div class="table-responsive-vertical mt-6">
            <table id="table" class="table-data table-hover table-mc-light-blue">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama <?= $sebutan_wilayah; ?></th>
                        <th>Kepala <?= $sebutan_wilayah; ?></th>
                    </tr>
                </thead>
                <tbody>
                <?php if($list_wilayah) : ?>
                    <?php foreach ($list_wilayah as $data) : ?>
                    <tr>
                        <td data-title='No'><?= ++$start; ?></td>
                        <td data-title='Nama'><?= $data->nama_dusun; ?></td>
                        <td data-title='Kategori'><?= $data->kepala_dusun; ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else : ?>
                    <tr>
                        <td colspan="3"><center>Data Tidak Tersedia</center></td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
